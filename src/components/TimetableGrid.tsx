...
export default function TimetableGrid({ events, currentDate, sectionName, niveau, filiereName, groupeName }: TimetableGridProps) {
    ...
    const checkTeacherAvailability = async (teacherName: string, newDate: string) => {
...
const handleDragEnd = async (result: DropResult) => {
    ...
    const exportPDF = async () => {
        ...
        const infoY = 35;
        const infoText = `Filière: ${filiereName || 'Non spécifiée'} • Section: ${sectionName || 'Non spécifiée'} • Niveau: ${niveau || 'Non spécifié'} • Groupe: ${groupeName || 'Non spécifié'}`;
        pdf.text(infoText, 15, infoY);
    ...
    const exportHTML = () => {
        ...
        html += `<div style="margin-bottom: 20px; padding: 10px; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">`;
        html += `<p style="margin: 5px 0;"><strong>Filière:</strong> ${filiereName || 'Non spécifiée'} • <strong>Section:</strong> ${sectionName || 'Non spécifiée'} • <strong>Niveau:</strong> ${niveau || 'Non spécifié'} • <strong>Groupe:</strong> ${groupeName || 'Non spécifié'}</p>`;
        html += `</div>`;
